...
function LoginAnimated() {
 ...
 const hairRef = useRef(null);
    ...
    function getCoord(e) {
        var carPos = emailRef.selectionEnd,
            div = MyDiv,
            span = MySpan,
            copyStyle = getComputedStyle(emailRef),
            emailCoords = {},
            caretCoords = {},
            centerCoords = {};
        ...
        div.textContent = emailRef.value.substr(0, carPos);
        span.textContent = emailRef.value.substr(carPos) || '.';
        div.appendChild(span);

        emailCoords = getPosition(emailRef); //console.log("emailCoords.x: " + emailCoords.x + ", emailCoords.y: " + emailCoords.y);
        caretCoords = getPosition(span); //console.log("caretCoords.x " + caretCoords.x + ", caretCoords.y: " + caretCoords.y);
        centerCoords = getPosition(mySVGRef); //console.log("centerCoords.x: " + centerCoords.x);
        svgCoords = getPosition(mySVGRef);
        screenCenter = centerCoords.x + mySVGRef.offsetWidth / 2; //console.log("screenCenter: " + screenCenter);
        ...
        TweenMax.to(eyeLRef, 1, { x: -eyeLX, y: -eyeLY, ease: Expo.easeOut });
        TweenMax.to(eyeRRef, 1, { x: -eyeRX, y: -eyeRY, ease: Expo.easeOut });
        TweenMax.to(noseRef, 1, { x: -noseX, y: -noseY, rotation: mouthR, transformOrigin: 'center center', ease: Expo.easeOut });
        TweenMax.to(mouthRef, 1, { x: -mouthX, y: -mouthY, rotation: mouthR, transformOrigin: 'center center', ease: Expo.easeOut });
        TweenMax.to(chinRef, 1, { x: -chinX, y: -chinY, scaleY: chinS, ease: Expo.easeOut });
        TweenMax.to(faceRef, 1, { x: -faceX, y: -faceY, skewX: -faceSkew, transformOrigin: 'center top', ease: Expo.easeOut });
        TweenMax.to(eyebrowRef, 1, { x: -faceX, y: -faceY, skewX: -eyebrowSkew, transformOrigin: 'center top', ease: Expo.easeOut });
        TweenMax.to(outerEarLRef, 1, { x: outerEarX, y: -outerEarY, ease: Expo.easeOut });
        TweenMax.to(outerEarRRef, 1, { x: outerEarX, y: outerEarY, ease: Expo.easeOut });
        TweenMax.to(earHairLRef, 1, { x: -outerEarX, y: -outerEarY, ease: Expo.easeOut });
        TweenMax.to(earHairRRef, 1, { x: -outerEarX, y: outerEarY, ease: Expo.easeOut });
        TweenMax.to(hairRef, 1, { x: hairX, scaleY: hairS, transformOrigin: 'center bottom', ease: Expo.easeOut });
    ...
    function onEmailInput(e) {
        ...
        if (curEmailIndex > 0) {
            if (mouthStatus == 'small') {
                mouthStatus = 'medium';
                TweenMax.to([mouthBGRef, mouthOutlineRef, mouthMaskPathRef], 1, { morphSVG: mouthMediumBGRef, shapeIndex: 8, ease: Expo.easeOut });
                TweenMax.to(toothRef, 1, { x: 0, y: 0, ease: Expo.easeOut });
                TweenMax.to(tongueRef, 1, { x: 0, y: 1, ease: Expo.easeOut });
                TweenMax.to([eyeLRef, eyeRRef], 1, { scaleX: 0.85, scaleY: 0.85, ease: Expo.easeOut });
            }
            if (value.includes('@')) {
                mouthStatus = 'large';
                TweenMax.to([mouthBGRef, mouthOutlineRef, mouthMaskPathRef], 1, { morphSVG: mouthLargeBGRef, ease: Expo.easeOut });
                TweenMax.to(toothRef, 1, { x: 3, y: -2, ease: Expo.easeOut });
                TweenMax.to(tongueRef, 1, { y: 2, ease: Expo.easeOut });
                TweenMax.to([eyeLRef, eyeRRef], 1, { scaleX: 0.65, scaleY: 0.65, ease: Expo.easeOut, transformOrigin: 'center center' });
            } else {
                mouthStatus = 'medium';
                TweenMax.to([mouthBGRef, mouthOutlineRef, mouthMaskPathRef], 1, { morphSVG: mouthMediumBGRef, ease: Expo.easeOut });
                TweenMax.to(toothRef, 1, { x: 0, y: 0, ease: Expo.easeOut });
                TweenMax.to(tongueRef, 1, { x: 0, y: 1, ease: Expo.easeOut });
                TweenMax.to([eyeLRef, eyeRRef], 1, { scaleX: 0.85, scaleY: 0.85, ease: Expo.easeOut });
            }
            ...
            mouthStatus = 'small';
            TweenMax.to([mouthBGRef, mouthOutlineRef, mouthMaskPathRef], 1, { morphSVG: mouthSmallBGRef, shapeIndex: 9, ease: Expo.easeOut });
            TweenMax.to(toothRef, 1, { x: 0, y: 0, ease: Expo.easeOut });
            TweenMax.to(tongueRef, 1, { y: 0, ease: Expo.easeOut });
            TweenMax.to([eyeLRef, eyeRRef], 1, { scaleX: 1, scaleY: 1, ease: Expo.easeOut });
        }
    }
    ...
    function coverEyes() {
        TweenMax.to(armLRef, 0.45, { x: -93, y: 2, rotation: 0, ease: Power1.easeOut });
        TweenMax.to(armRRef, 0.45, { x: -93, y: 2, rotation: 0, ease: Power1.easeOut, delay: 0.1 });
    }

    function uncoverEyes() {
        TweenMax.to(armLRef, 1.35, { y: 220, ease: Power1.easeOut });
        TweenMax.to(armLRef, 1.35, { rotation: 105, ease: Power1.easeOut, delay: 0.1 });
        TweenMax.to(armRRef, 1.35, { y: 220, ease: Power1.easeOut });
        TweenMax.to(armRRef, 1.35, { rotation: -105, ease: Power1.easeOut, delay: 0.1 });
    }

    function resetFace() {
        TweenMax.to([eyeLRef, eyeRRef], 1, { x: 0, y: 0, ease: Expo.easeOut });
        TweenMax.to(noseRef, 1, { x: 0, y: 0, scaleX: 1, scaleY: 1, ease: Expo.easeOut });
        TweenMax.to(mouthRef, 1, { x: 0, y: 0, rotation: 0, ease: Expo.easeOut });
        TweenMax.to(chinRef, 1, { x: 0, y: 0, scaleY: 1, ease: Expo.easeOut });
        TweenMax.to([faceRef, eyebrowRef], 1, { x: 0, y: 0, skewX: 0, ease: Expo.easeOut });
        TweenMax.to([outerEarLRef, outerEarRRef, earHairLRef, earHairRRef, hairRef], 1, { x: 0, y: 0, scaleY: 1, ease: Expo.easeOut });
    }
...
emailRef.current.addEventListener('focus', onEmailFocus);
emailRef.current.addEventListener('blur', onEmailBlur);
emailRef.current.addEventListener('input', onEmailInput);
passwordRef.current.addEventListener('focus', onPasswordFocus);
passwordRef.current.addEventListener('blur', onPasswordBlur);
// TweenMax.set(armLRef, { x: -93, y: 220, rotation: 105, transformOrigin: 'top left' });
// TweenMax.set(armRRef, { x: -93, y: 220, rotation: -105, transformOrigin: 'top right' });
    ...
    //     armRRef = document.querySelector('.armRRef'),
    ...
    return (
        <div className="all-Login">
            <form>
                <div className="svgContainer" ref={mySVGRef}>
                    <div>
                        <svg className="mySVG" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 200 200">
                            ...
                            <g className="earL" ref={outerEarLRef}>
                                <g className="outerEar" ref={outerEarLRef} fill="#ddf1fa" stroke="#3a5e77" stroke-width="2.5">
                            ...
                            <g className="earR" ref={outerEarRRef}>
                                <g className="outerEar" ref={outerEarRRef} fill="#ddf1fa" stroke="#3a5e77" stroke-width="2.5">
                            ...
                            <g className="mouth" ref={mouthRef}>
                                ...
                                <path clip-path="url(#mouthMask)" className="tooth" ref={toothRef} style={{ fill: '#FFFFFF' }} d="M106,97h-4c-1.1,0-2-0.9-2-2v-2h8v2C108,96.1,107.1,97,106,97z" />
                                <path
                ...
                <div className="inputGroup inputGroup1">
                    ...
                    <input type="text" id="email" ref={emailRef} className="email" maxLength={256} />
